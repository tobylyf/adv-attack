...
from mixed_folder import PartialDatasetFolder, MixedPartialDatasetFolder
...
parser.add_argument('-p', '--prob', default=0.5, type=float)
...
IMAGENET_ADV = '/DATA5_DB8/data/yfli/datasets/ImageNet_PGD16/ResNet50_224_new/adv/'
IMAGENET_CLEAN = '/DATA5_DB8/data/yfli/datasets/ImageNet_PGD16/ResNet50_224_new/clean/'
IMAGENET_VAL = '/DATA5_DB8/data/yfli/datasets/ImageNet_val_PGD/ResNet50_224_new/adv/'
...
if True:  # for debugging
    train_set = MixedPartialDatasetFolder(os.path.join(IMAGENET_ADV, 'train/'), os.path.join(IMAGENET_CLEAN, 'train/'),
                                          is_train=True, p=args.prob, loader=np.load, extensions=['.npy'],
                                          transform=torch.from_numpy)
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
# val_set = datasets.ImageFolder(os.path.join(IMAGENET_VAL, 'val/'), transform=transform)
val_set = PartialDatasetFolder(os.path.join(IMAGENET_VAL, 'val/'), is_train=False, loader=np.load, extensions=['.npy'],
                               transform=torch.from_numpy)
...
for epoch in range(args.epochs):
    ...
    for step, (logits, labels) in enumerate(train_loader):
