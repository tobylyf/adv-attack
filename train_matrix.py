...
parser = argparse.ArgumentParser(description='Adversarial Matrix Training')
parser.add_argument('--gpu', '--gpu_ids', type=str, required=True)
parser.add_argument('-b', '--batch_size', default=256, type=int)
parser.add_argument('--pairing', deafult=None)
parser.add_argument('--lr', '--learning_rate', default=0.0001, type=float)
parser.add_argument('--momentum', default=0.9, type=float)
parser.add_argument('--wd', '--weight_decay', default=1e-4, type=float)
parser.add_argument('--epochs', default=50, type=int)
parser.add_argument('-f', '--print_freq', default=50, type=int)
parser.add_argument('--name', default='matrix', type=str)
...
net = nn.Linear(1000, 1000, bias=False)
net.to(device)
...
for epoch in range(args.epochs):
    ...
    for step, (logits_adv, logits_clean, labels) in enumerate(train_loader):
        ...
        output = net(logits_adv)

        if args.pairing is not None:
            loss = criterion_l1(output, logits_clean)
        else:
